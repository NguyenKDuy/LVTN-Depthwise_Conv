...
module pre_process (
    ...
    localparam [14:0] K_MULT = 14'd16449;
─────────────────────────────────────────────────────────────────────
    // STAGE 0 – Tách kênh, đăng ký input vào DSP A/B reg
    //   Đây là "pre-register" để tool có thể map vào DSP48 A1/B1 register,
    //   giúp giảm fanout và setup time cho DSP.
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
    // STAGE 1 – Nhân trong DSP, capture vào P-register của DSP48
    //
    //   prod = x * 16449   (8-bit × 14-bit = 22-bit unsigned, store 23-bit)
    //
    //   Attribute (* use_dsp = "yes" *) báo Vivado ưu tiên map vào DSP48.
    //   Vivado tự động bật DSP P-register khi timing yêu cầu.
    //
    //   Hằng K = 16449 = 0x4041. Với DSP48E2:
    ...
    always @(posedge i_clk) begin
        if (!i_rst_n) begin
            ...
            s1_valid <= s0_valid;
            // DSP P-reg: kết quả nhân được giữ trong DSP, không route ra fabric ngay
    ...
    //
    //   Sau khi prod đã được register trong DSP P-reg (S1),
    //   chỉ còn:
    //     • shift >> 11  : chỉ là wire nối bit (0 logic)
    //     • subtract 1024: 1 adder nhỏ 12-bit
    //     • pack 192-bit : wiring
    //   → Path này rất ngắn, dễ meet timing dù Fmax cao.
    // ─────────────────────────────────────────────────────────────────────
...
module post_process (
    ...
    // ── Input ← IP_top ───────────────────────────────────────────────────
    input  wire         i_valid,
    input  wire [191:0]  i_data,         // {B[47:32], G[31:16], R[15:0]} Q6.10
    input  wire         i_tlast,         // Tín hiệu last của AXI4-Stream

    // ── AXI4-Stream Master → DMA ─────────────────────────────────────────
    output reg          m_axis_tvalid,
    output reg  [127:0]  m_axis_tdata,   // {X[31:24], B[23:16], G[15:8], R[7:0]}
...
// ─────────────────────────────────────────────────────────────────────
    // STAGE 0 – Tách kênh, đăng ký vào DSP A/B pre-register
    //   Tách Q6.10 từ i_data và đăng ký trước khi đưa vào DSP.
    //   Điều này cho phép tool map vào DSP48 A1-reg hoặc B1-reg,
    //   loại bỏ combinational path dài từ input đến DSP multiplier.
    // ─────────────────────────────────────────────────────────────────────
    ...
    //   Lưu 24-bit (range đủ: max |q610|=1024, 1024*255=261120 < 2^18)
    //
    //   Với DSP48E2: A=q610(16-bit), B=255(9-bit), OPMODE=multiply
    //   P-register bật → kết quả lưu trong DSP, không route ra fabric ngay.
    // ─────────────────────────────────────────────────────────────────────
    ...
    always @(posedge i_clk) begin
        if (!i_rst_n) begin
            ...
            s1_valid  <= s0_valid;
            s1_tlast  <= s0_tlast;
            // DSP P-reg: multiply với s0 đã pre-registered → path ngắn nhất
    ...
    //
    //   Path: chỉ còn 1 adder 14-bit + 2 comparator → rất ngắn.
    // ─────────────────────────────────────────────────────────────────────
 
    // Arithmetic right shift: lấy bit [23:11] (giữ sign bit [23])
    // s1_prod là signed 24-bit, [23:11] cho 13-bit signed (đã shift >>> 11)
    ...
    // Mở rộng 1 bit để tránh overflow khi cộng 128
