module line_buffer (
    ...
    // Thay v? 1 m?ng 256-bit, ta chia th�nh 4 m?ng 64-bit
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // 2. Nh�n b?n Write Pointer (Replication)
    // D�ng thu?c t�nh DONT_TOUCH �? Vivado kh�ng g?p ch�ng l?i th�nh 1
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // 3. C?p nh?t Write Logic �?ng b? cho c�c b?n sao
    // -------------------------------------------------------------------------
    always @(posedge i_clk) begin
        if (!i_rst_n) begin
            ...
            // M?i con tr? ch? �i?u khi?n ghi v�o "ph�n khu" c?a n�
    ...
    // -------------------------------------------------------------------------
    // 4. Read Logic (T��ng t? c� th? nh�n b?n rdPntr n?u c?n)
    // -------------------------------------------------------------------------
    always @(posedge i_clk) begin
        if (!i_rst_n) begin
            rdPntr_reg <= 8'd0;
    ...
    always @(posedge i_clk) begin
        if (!i_rst_n) begin
            ...
            // Gom d? li?u t? c�c sub-blocks khi �?c
