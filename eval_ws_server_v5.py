...
class ServerConfig:
    ...
    SAVE_FIGURES = False   # tắt để tăng tốc
    SAVE_PER_SAMPLE_PNG = False
...
async def handle_client(websocket, evaluator):
    peer = websocket.remote_address
    log.info(f"🔌 Kết nối mới từ {peer}")
    n_received = 0
    loop = asyncio.get_running_loop()

    # Semaphore: cho phép pipeline board gửi nhanh,
    # server evaluate tuần tự không mất frame
    eval_queue: asyncio.Queue = asyncio.Queue(maxsize=0)  # không giới hạn

    async def eval_worker():
        """Nhận frame từ queue, evaluate, gửi ACK – tuần tự."""
        while True:
            ...
            try:
                ...
                log.error(f"  ❌ Parse lỗi frame #{frame_num}: {e}")
                try:
            ...
            sid = header.get("sample_id", f"frame_{frame_num}")
            log.info(f"  ▶ Đánh giá [{sid}]")
            try:
                result = await loop.run_in_executor(
                    None, evaluator.evaluate, header, png_bytes)
                ack = {"status": "ok", "sample_id": sid,
                       "timestamp": datetime.now().isoformat(),
                       "eval": result}
                await websocket.send(json.dumps(ack, ensure_ascii=False))
                log.info(f"  ✅ ACK #{frame_num} [{sid}]")
            except Exception as e:
                log.error(f"  ❌ Evaluate [{sid}]: {e}", exc_info=True)
                try:
                    await websocket.send(json.dumps(
                        {"status": "error", "sample_id": sid,
                         "msg": str(e)}))
                except Exception:
                    pass

    # Khởi động eval worker task
    worker_task = asyncio.create_task(eval_worker())

    try:
        while True:
            try:
                data = await asyncio.wait_for(
                    websocket.recv(), timeout=120)
            except asyncio.TimeoutError:
                log.warning(f"  ⏰ Timeout 120s – {peer}")
                break
            ...
            n_received += 1
            log.info(f"  📦 Frame #{n_received}: {len(data)/1024:.1f} KB")
            await eval_queue.put((n_received, data))   # ← không block

    except websockets.exceptions.ConnectionClosedOK:
        log.info(f"🔌 {peer} đóng bình thường ({n_received} frame)")
    except websockets.exceptions.ConnectionClosedError as e:
        log.warning(f"🔌 {peer} mất kết nối: {e}")
    except Exception as e:
        log.error(f"❌ Lỗi: {e}", exc_info=True)
    finally:
        await eval_queue.put(None)   # dừng worker
        await worker_task            # chờ xử lý hết
        log.info(f"🔌 Handler {peer} kết thúc – {n_received} frame")
...
# 12. MAIN SERVER
# ─────────────────────────────────────────────────────────────────────────────
async def main_server(cfg):
    """
    Khởi động WebSocket server với persistent connection handler.
    Thay đổi duy nhất so với bản gốc: handler → handle_client mới.
    """
    ...
    log.info("═" * 60)
    log.info(f"  🚀 WebSocket Server (persistent connection mode)")
