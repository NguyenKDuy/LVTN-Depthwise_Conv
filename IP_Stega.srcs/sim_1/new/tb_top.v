...
module tb_top;
    ...
    // File Handles (M?ng ð? qu?n l? t?p trung)
    ...
    // =========================================================================
    initial begin
        // 1. T?o folder (Dùng l?nh shell chu?n)
        $display("--- PREPARING LOG DIRECTORIES ---");
        $system("mkdir -p img1_logs img2_logs");
        #100;

        // 2. M? toàn b? 18 file cùng lúc ð? tránh l?i descriptor
        ...
        // 5. Ðóng toàn b? file khi k?t thúc h?n mô ph?ng
        for(i=0; i<9; i=i+1) begin
            $fclose(f1[i]);
            $fclose(f2[i]);
        end
    ...
    always @(posedge i_clk) begin
        if (!i_rst_n) begin
            ...
            // T? ð?ng reset counter khi ð?i Stage
            if (top_inst.u_rd_fsm_control.o_stage != stage_delayed) begin
                for(i=0; i<9; i=i+1) cnt[i] <= 0;
            end else begin
                
                // Logic ghi log s? d?ng Macro ð? tránh sai sót
                ...
                // Ð?c thù SO không có stage nên log riêng
